...
export class CreateEvaluationService implements CreateEvaluationUsecase {
  ...
  async perform(params: CreateEvaluationUsecase.Params): Promise<CreateEvaluationUsecase.Response> {
    ...
    await this.updateClientService.perform({ uid: createdClient.uid, attrs: { 'lastEvaluatedAt': new Date() } })

    return await this.evaluationRepository.create({ uid, userUid, client: createdClient, bioimpedance, measurements, nutricionistForm })
  }
}
